...
PACKAGE := pip_ui
PYTHON_TARGETS := pip_ui tests
PYLINT_MAIN_TARGETS := pip_ui
...
ABOUT_FILE := pip_ui/__about__.py
...
dont-be-lazy:
	...
	@$(UV) dont_be_lazy --root . --no-color scan pip_ui --no-config-suppressions || true

.PHONY: pydoc-docs
pydoc-docs:
	@$(UV) pydoc_fork pip_ui -o ./pydoc/
