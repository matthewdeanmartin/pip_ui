...
def collect_argv_warnings(argv: list[str]) -> list[str]:
    ...
    if "--trusted-host" in argv:
        warnings.append(
            "--trusted-host is set. pip will skip TLS validation for that host. "
...
def explain_pip_error(stderr: str) -> list[str]:
    ...
    if "permission denied" in lower or "errno 13" in lower:
        hints.append(
            "Permission denied. You may be installing into a system location. Try a virtual environment "
            "or --user."
        )
